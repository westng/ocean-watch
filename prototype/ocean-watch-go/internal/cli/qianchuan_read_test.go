...
func TestRunnerQianchuanReadShadowAssemblesTokenAndSDKAdapters(t *testing.T) {
	root := t.TempDir()
	codexHome := filepath.Join(root, "codex-home")
	stateRoot := filepath.Join(codexHome, "ads-plan-monitor", "state")
  ...
  "qianchuan_product_templates": {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if result["advertiser_id"] != "1000000000000001" {
				t.Fatalf("unexpected Shadow advertiser: %#v", result)
			}
			...
			if bytes.Contains(stdout.Bytes(), []byte("stats_info")) || bytes.Contains(stdout.Bytes(), []byte("stat_cost")) ||
				...
				t.Fatalf("SDK finance escaped stable Shadow DTO: %s", stdout.String())
			}
	...
	if fallback.args != nil {
		t.Fatalf("Qianchuan Shadow invoked Python fallback: %v", fallback.args)
	}
	...
	transport.mu.Unlock()
	if len(calls) != 10 {
		t.Fatalf("Qianchuan Shadow SDK calls = %d, want 10: %#v", len(calls), calls)
	}
	wantPaths := []string{
	...
	for index, call := range calls {
		if call.Path != wantPaths[index] || call.Host != wantHosts[index] || call.Token != "fixture-qianchuan-token" {
			t.Fatalf("Qianchuan Shadow SDK call %d changed: %#v", index, call)
		}
	}
	planCall := calls[2]
	if planCall.Query.Get("start_time") != "2026-07-25 00:00:00" ||
		...
		t.Fatalf("Qianchuan Shadow plan-list default changed: %#v", planCall)
	}
...
func TestDefaultRouteKeepsQianchuanReadsOnPython(t *testing.T) {
	for _, command := range []string{
		"qc-products list", "qc-products search", "qc-plans list", "qc-plans show", "qc-plans materials",
		"qc-materials authorized-creators", "qc-materials creator-videos",
	} {
