...
func TestRunnerMarketingMaterialShadowUsesSDKAndFrozenDefaults(t *testing.T) {
	root := t.TempDir()
  ...
  "future_field": {"must_remain": true}
	...
	if fallback.args != nil {
		t.Fatalf("Marketing material Shadow invoked Python fallback: %v", fallback.args)
	}
...
func TestRunnerMarketingMaterialRejectsInvalidArgumentsBeforeLocalState(t *testing.T) {
	...
	var mu sync.Mutex
	runner := Runner{
		Routes: manifest, Fallback: fallback, Credentials: credentials,
	...
	if configReads != 0 || len(credentials.reads) != 0 || credentials.writes != 0 || fallback.args != nil {
		t.Fatalf("invalid args crossed local/runtime boundary: config=%d credential=%v/%d fallback=%v",
			configReads, credentials.reads, credentials.writes, fallback.args)
	}
}

func TestDefaultRouteKeepsMarketingMaterialsOnPython(t *testing.T) {
