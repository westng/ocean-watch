...
func TestRunnerRoutesQianchuanPlanActionsWithoutFallbackOrMarketing(t *testing.T) {
	...
	if code != 0 || qianchuan.createCalls != 1 || marketing.calls != 0 || fallback.args != nil {
		t.Fatalf("Qianchuan plan route crossed runtime boundary: exit=%d qc=%d marketing=%d fallback=%v output=%s",
			code, qianchuan.createCalls, marketing.calls, fallback.args, stdout.String())
	}
...
func TestRunnerKeepsMarketingCreateOutOfQianchuanRuntime(t *testing.T) {
	manifest := qianchuanPlanShadowManifest(t, "plans create")
...
func TestRunnerRoutesQianchuanMutationBeforeReadRuntime(t *testing.T) {
	manifest := qianchuanPlanShadowManifest(t, "qc-plans update-budget")
	...
	if code != 0 || mutations.calls != 1 || fallback.args != nil {
		t.Fatalf("Qianchuan mutation did not use write runtime: exit=%d calls=%d fallback=%v output=%s",
			code, mutations.calls, fallback.args, stdout.String())
	}
...
func TestQianchuanWriteCommandsRemainPythonByDefault(t *testing.T) {
	for _, command := range []string{
		...
		if !ok || runtime != application.RuntimePython {
			t.Fatalf("default production route for %s = %q, want Python", command, runtime)
		}
	}
}

func qianchuanPlanShadowManifest(t *testing.T, commands ...string) application.RouteManifest {
	t.Helper()
