...
func TestOperationJournalPreservesPythonExtensions(t *testing.T) {
	root := t.TempDir()
...
func TestOperationJournalAcceptsPythonCreatorBatchJobKeys(t *testing.T) {
	root := t.TempDir()
	store := OperationJournalStore{Root: root}
	...
	if !ok || job.ProjectID != "9007199254740995" {
		t.Fatalf("Python creator journal job was not preserved: %#v", loaded.Jobs)
	}
	if string(job.Extra["plan_template"]) != `"巨量营销-模板"` {
		t.Fatalf("Python journal extension field changed: %s", job.Extra["plan_template"])
	}
}
...
func TestAdvertiserLockNamesMatchPythonProtocol(t *testing.T) {
	tests := []struct {
