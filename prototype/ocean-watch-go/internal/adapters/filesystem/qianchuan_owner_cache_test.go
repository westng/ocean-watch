...
func TestQianchuanOwnerHintCacheMatchesPythonSchemaTTLAndLock(t *testing.T) {
	...
	if len(hints) != 1 || hints["6000000000000001"].AwemeShowID != "visible-1" {
		t.Fatalf("TTL or Python schema compatibility changed: %#v", hints)
	}
	...
	if _, err := os.Stat(path + ".lock"); err != nil {
		t.Fatalf("Python-compatible lock suffix is missing: %v", err)
	}
