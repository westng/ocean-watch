...
func (state LocalState) Snapshot(
	...
	legacy, err := state.Credentials.Read(ctx, "oceanengine-oauth")
	if err != nil {
		return CredentialSnapshot{}, err
	}
	...
	status["has_developer_id"] = !configuration.Missing(legacy["developer_id"])
	...
	return CredentialSnapshot{
		...
		Legacy: configuration.CloneMap(legacy), ChannelStatus: channelStatus, Status: status,
	}, nil
}
