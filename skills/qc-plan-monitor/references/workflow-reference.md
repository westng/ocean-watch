# Qianchuan Workflow Reference

> 按需详细合同。`../SKILL.md` 的快速路由矩阵具有入口优先级；仅在已确定为复杂 CLI 分支后读取本文件。
...
| Discover an uncommon Qianchuan CLI capability after no fast route matched | MCP `get_capabilities` with `channel=qianchuan` |
