...
- Interactive creator lookup uses `search_key_words` with a visible Douyin ID or creator name, then requires an exact local `aweme_show_id` match and uses the returned numeric `aweme_id`. Batch work owner hints instead search with the third-party numeric `author.uid` and require the official row's numeric `aweme_id` to match exactly; a changed or missing visible ID is only auxiliary and must not force a broad creator-video scan.
