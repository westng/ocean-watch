...
- Interactive creator lookup uses `search_key_words` with a visible Douyin ID or creator name, then requires an exact local `aweme_show_id` match and uses the returned numeric `aweme_id`. Batch work owner hints use the third-party visible `author.unique_id` as `search_key_words` and require the official row's numeric `aweme_id` to match the third-party numeric `author.uid`; the numeric UID must never replace the visible search value, while a changed visible ID in the official response does not invalidate a matching numeric identity or justify a broad creator-video scan.
