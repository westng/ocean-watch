...
| Discover an uncommon Marketing CLI capability after no fast route matched | MCP `get_capabilities` with `channel=marketing` |
...
- Membership-only requests such as `我负责的账户`, `我常用的账户`, `我管的户`, or a contextual equivalent ask which accounts are in the user's working scope. Call MCP `list_managed_accounts` during the current turn. Read only the enabled local registry records by default; it must not resolve credentials, refresh a Token, or call any official report API.
