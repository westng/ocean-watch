...
def resolve_accounts(config, accounts_arg):
    ...
    accounts = []
    account_id = create_plan.get_path(config, "account.advertiser_id")
    if not create_plan.is_missing(account_id):
    ...
    for item in config.get("accounts") or []:
        if isinstance(item, dict):
            ...
            item_id = item
        if not create_plan.is_missing(item_id):
...
def compact_response(response):
    ...
    project_id = create_plan.get_path(response, "data.project_id")
    promotion_id = create_plan.get_path(response, "data.promotion_id")
    if project_id:
...
def compact_and_dedupe_videos(rows):
    ...
    for row in rows:
        ...
        if create_plan.is_missing(video_id):
            skipped.append({
...
def validate_ad_get(base_url, token, advertiser_id, videos, args):
    ...
    for video_group in chunked([video["video_id"] for video in videos], args.ad_get_batch_size):
        ...
        for item in create_plan.get_path(response, "data.list", []) or []:
...
def fetch_cover(base_url, token, advertiser_id, video, attempts, wait_sec):
    ...
    for attempt in range(1, attempts + 1):
        ...
        data_list = create_plan.get_path(response, "data.list", []) or []
        if response.get("code") == 0 and data_list:
            cover_id = data_list[0].get("id")
            if not create_plan.is_missing(cover_id):
                return {
                    "ok": True,
                    "video_id": video_id,
                    "video_cover_id": cover_id,
                    "attempts": attempt,
                    "status": create_plan.get_path(response, "data.status"),
                    "request_id": response.get("request_id"),
                }
        if attempt < attempts:
            time.sleep(wait_sec)
    ...
    return {
        ...
        "video_id": video_id,
        "attempts": attempts,
        "status": create_plan.get_path(last_response, "data.status"),
...
def build_group_payloads(base_config, advertiser_id, videos, args, group_number):
    ...
    config["materials"]["video_cover_ids"] = {
        ...
        for video in videos
        if not create_plan.is_missing(video.get("video_cover_id"))
    }
...
def process_group(base_config, base_url, token, advertiser_id, videos, group_number, args):
    ...
    project_response = create_plan.post_json(base_url, token, PROJECT_CREATE_PATH, project_payload)
    project_id = create_plan.get_path(project_response, "data.project_id")
    result["project_response"] = compact_response(project_response)
    if not project_id:
        result["status"] = "project_failed"
        return result

    promotion_payload["project_id"] = project_id
    promotion_response = create_plan.post_json(base_url, token, PROMOTION_CREATE_PATH, promotion_payload)
    promotion_id = create_plan.get_path(promotion_response, "data.promotion_id")
    result["project_id"] = project_id
    result["promotion_response"] = compact_response(promotion_response)
    if promotion_id:
        result["promotion_id"] = promotion_id
...
def process_account(raw_config, config_path, advertiser_id, args):
    ...
    if create_plan.is_missing(base_url) or create_plan.is_missing(token):
