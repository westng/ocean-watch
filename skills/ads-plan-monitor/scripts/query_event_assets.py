...
def main():
    ...
    parser.add_argument("--asset-type", default="THIRD_EXTERNAL")
    parser.add_argument("--asset-id", action="append", type=int)
    ...
    filtering = {}
    if args.asset_type:
        filtering["asset_type"] = args.asset_type
    if args.asset_id:
        filtering["asset_ids"] = args.asset_id
    response = get_json(
        get_path(config, "api.base_url"),
        get_path(config, "api.access_token"),
        "/2/tools/event/all_assets/list/",
        {
    ...
    result = {
        "endpoint": "/2/tools/event/all_assets/list/",
        "params": {
            "advertiser_id": get_path(config, "account.advertiser_id"),
            "filtering": filtering,
            "page": args.page,
