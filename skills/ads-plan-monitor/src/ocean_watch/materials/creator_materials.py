...
HOMEPAGE_ENDPOINT = "/2/file/video/aweme/get/"
...
def normalize_relation(row, advertiser_id, minimum_remaining_days=1, now=None):
    ...
    aweme_id = official_string(row.get("aweme_id"))
    item_id = official_id(video.get("item_id"))
    video_id = official_string(video.get("video_id"))
    cover_id = official_string(video.get("video_cover_id"))
...
def fetch_candidates(
    ...
    *,
    ...
    normalized_aweme_ids = [official_string(value) for value in (aweme_ids or [])]
    ...
    seen = set()
    for row in rows:
        if normalized_aweme_ids and official_string(row.get("aweme_id")) not in normalized_aweme_ids:
            continue
        row_item_id = official_id((row.get("video_info") or {}).get("item_id"))
        if normalized_item_ids and row_item_id not in normalized_item_ids:
            continue
...
def fetch_homepage_videos(
    ...
    *,
    ...
    normalized_advertiser_id = official_id(advertiser_id)
    normalized_aweme_id = official_string(aweme_id)
    if not normalized_advertiser_id:
        raise CreatorMaterialError("invalid_advertiser_id", "advertiser_id must be a decimal string")
    if not normalized_aweme_id:
        raise CreatorMaterialError("invalid_aweme_id", "aweme_id is required")
    ...
    while True:
        ...
        if response.get("code") != 0:
            raise CreatorMaterialError(
                "creator_homepage_query_failed",
                str(response.get("message") or "creator homepage video query failed"),
                {"code": response.get("code"), "request_id": response.get("request_id")},
            )
        rows.extend((response.get("data") or {}).get("list") or [])
        if page >= response_total_pages(response):
    ...
    seen = set()
    for row in rows:
        item_id = official_id(row.get("item_id"))
        video_id = official_string(row.get("video_id"))
        cover_id = official_string(row.get("video_cover_id"))
        ...
        videos.append({
            "channel": "marketing",
            "owner_advertiser_id": normalized_advertiser_id,
            "source_type": "CREATOR_HOMEPAGE",
            "creator_id": normalized_aweme_id,
            "item_id": item_id,
            "video_id": video_id,
            "video_cover_id": cover_id,
...
def select_candidates(candidates, item_ids, max_materials=5):
    ...
    limit = int(max_materials) if max_materials is not None else None
    if limit is not None and (limit < 1 or len(requested) > limit):
        raise CreatorMaterialError(
...
def select_latest_candidates(candidates, max_materials=5):
    ...
    limit = int(max_materials) if max_materials is not None else len(selected_group)
...
def apply_to_promotion_payload(promotion_payload, selected_candidates):
    ...
    existing_aweme_id = official_string(native_setting.get("aweme_id"))
