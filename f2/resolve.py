...
def map_video_response(video, expected_work_id):
    ...
    if detail.get("aweme_type") == 0:
        video_result["video_info_url"] = str(
            first_value(detail, "video", "bit_rate", 0, "play_addr", "url_list", 0) or ""
        )
    return {
        "code": 200,
        "message": "数据获取成功",
...
async def fetch_many(
    ...
    *,
    ...
    if fetch_factory is None:
        ...
        fetch_video = fetch_factory(kwargs)

    async def fetch(work_id, attempts):
        attempt_started_at = time.monotonic()
        async with semaphore:
            try:
                video = await asyncio.wait_for(fetch_video(work_id), timeout=per_work_timeout)
                metadata = map_video_response(video, work_id)
                error = None
            except asyncio.TimeoutError:
                metadata = None
                error = {"code": "f2_work_timeout", "message": "F2 work metadata query timed out"}
            except Exception:
                metadata = None
                error = {
        ...
        attempts.setdefault(work_id, []).append(time.monotonic() - attempt_started_at)
        return work_id, metadata, error

    async def run_round(round_work_ids, attempts):
        tasks = {asyncio.create_task(fetch(work_id, attempts)): work_id for work_id in round_work_ids}
    ...
    try:
        ...
        retry_ids = [work_id for work_id in work_ids if work_id in errors and time.monotonic() < deadline]
        retry_started_at = time.monotonic()
        if retry_ids:
            for work_id, metadata, error in await run_round(retry_ids, attempts):
                if metadata is not None:
                    results[work_id] = metadata
