...
计划模板采用“公共默认模板 + 业务模板”结构。公共模板只提供共享参数；每个可执行的业务模板都必须明确绑定广告主 ID、平台、流量来源、商品 ID 和商品名称，并通过 `copy_materials.titles` 配置创建单元所用的文案素材。创建计划时会校验模板归属，防止把一个广告主的商品、链接和文案配置误投到另一个账户。
