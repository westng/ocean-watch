...
Use `scripts/token_manager.py --sync-accounts` to call the official OAuth authorized-account endpoint. Distinguish all OAuth account subjects from actual advertisers: only valid rows with `account_type=ADVERTISER` belong in `authorized_advertiser_ids`.
