...
If an API returns `40002` / "No permission to operate account", check `scripts/token_manager.py --status`. If `advertiser_id_authorized` is false, the current `account.advertiser_id` was not included in the OAuth authorization; ask the user to re-authorize with that account selected or switch config to an authorized advertiser ID.
