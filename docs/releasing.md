...
git tag -s vX.Y.Z -m "Ocean Watch vX.Y.Z"
git push origin vX.Y.Z
```

`Release` 工作流会重新执行编译、Ruff、Bandit 和全部测试，然后在全新 Ubuntu 环境构建、验证、证明并发布产物。Tag 必须指向 `main` 可达的提交；版本、Tag 或 Changelog 任一不一致都会在创建 GitHub Release 前失败。构建任务只有仓库只读权限，单独的发布任务不执行仓库代码，只下载已验证产物、重新校验 `SHA256SUMS` 并写入 GitHub Release。
