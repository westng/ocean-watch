...
```text
巨量营销-1234567890123456-示例商品-9876543210987654-混剪素材
巨量营销-1234567890123456-示例商品-9876543210987654-原生素材
```

已有模板名称不自动改名，避免破坏脚本参数、活动模板指针和用户习惯。新向导按渠道、广告主、商品和素材模式绑定自动生成完整名称，混剪和原生模式使用不同模版类型，用户不能输入自由名称。
...
任何迁移失败都不能留下半写入配置。后续 Schema v5 已删除 `active_plan_template`；所有创建入口必须显式选择业务模板。需要素材迁移确认的旧模板在确认前不能被新创建入口部分执行。
