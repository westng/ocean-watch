...
> 文档状态：历史设计，已被 [`ocean-watch-plugin-target-architecture-v3.md`](ocean-watch-plugin-target-architecture-v3.md) 取代；不得继续作为当前实施依据
>
> 文档属性：AI 辅助历史架构设计，用于保留 V2 决策背景和已实现来源，不是当前产品行为或 V3 目标的权威来源
>
> 权威边界：当前行为仍以仓库代码、测试、Plugin 清单、`qc-plan-monitor` Skill 和正式文档为准；后续整改以 V3 目标架构和实施计划为准
>
> 证据核实日期：2026-08-16
>
> 验证边界：本轮不读取真实凭据、不调用官方业务 API、不构建或安装 Plugin、不创建 Tag 或 GitHub Release

> 取代原因：2026-08-18 真实混合类型预检反证了“同一达人一组”“快照按达人索引”和“达人+商品即可匹配已有计划”等关键前提。保留本文件是为了追溯历史，不表示其中的完成状态仍适用于当前目标架构。
