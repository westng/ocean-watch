...
def validate_release_changelog(changelog, version):
    ...
    if release_match is None:
        raise ReleaseError(f"CHANGELOG.md has no dated release heading for {version}")
    ...
    if substantive_lines:
        raise ReleaseError("CHANGELOG.md 未发布段落在发布前必须为空")
...
def validate_versions(root, tag=None):
    ...
    if tag:
        ...
        if match.group("version") != versions["project"]:
            raise ReleaseError(f"tag {tag} does not match project version {versions['project']}")
...
def build_parser():
    parser = argparse.ArgumentParser(description="Validate and publish Ocean Watch releases.")
