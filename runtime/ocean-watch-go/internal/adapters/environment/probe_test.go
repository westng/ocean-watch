...
func TestPythonCheckUsesResolvedLauncherRuntime(t *testing.T) {
	probe := Probe{PythonResolver: python.Resolver{
		...
		Run: func(_ context.Context, _ string, arguments []string) (python.CommandResult, error) {
			if arguments[len(arguments)-1] == "import importlib.metadata; print(importlib.metadata.version('f2'))" {
				return python.CommandResult{Stdout: python.RequiredF2Version}, nil
			}
...
func TestF2CheckRequiresPinnedPackageVersion(t *testing.T) {
	installed := "0.0.1.6"
	probe := Probe{PythonResolver: python.Resolver{
		Getenv:   func(string) string { return "/synthetic/python" },
		LookPath: func(name string) (string, error) { return name, nil },
		Run: func(_ context.Context, _ string, arguments []string) (python.CommandResult, error) {
			if arguments[len(arguments)-1] == "import importlib.metadata; print(importlib.metadata.version('f2'))" {
				return python.CommandResult{Stdout: installed}, nil
			}
			return python.CommandResult{Stdout: "3.12.4"}, nil
		},
	}}
	...
	ready := probe.F2(context.Background())
	if ready["status"] != "ready" || ready["version"] != python.RequiredF2Version {
		t.Fatalf("unexpected pinned F2 check: %#v", ready)
	}
}

func TestPlatformCheckMatchesStableMachineAndReleaseShape(t *testing.T) {
	probe := Probe{
