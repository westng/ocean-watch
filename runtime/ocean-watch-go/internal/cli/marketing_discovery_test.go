...
func newMarketingDiscoveryGoManifest(t *testing.T) application.RouteManifest {
	t.Helper()
...
func newMarketingDiscoveryRunner(
	...
	return Runner{
		Routes: newMarketingDiscoveryGoManifest(t), Stdout: stdout,
...
func TestRunnerMarketingDiscoveryRuntimeUsesGeneratedSDK(t *testing.T) {
	root := t.TempDir()
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			runner := newMarketingDiscoveryRunner(
				t, root, configPath, transport, store, credentials, stdout,
			)
...
func TestRunnerMarketingCitiesBOMChineseHeaderAndAtomicWrite(t *testing.T) {
	...
	store := &marketingDiscoveryConfigStoreSpy{path: configPath}
	stdout := new(bytes.Buffer)
	runner := newMarketingDiscoveryRunner(t, root, configPath, transport, store, credentials, stdout)
	...
	if len(credentials.reads) != 1 || credentials.reads[0] != "oceanengine-auth-marketing-marketing-auth-r1" ||
		...
		t.Fatalf("cities resolved more than one token lease: reads=%v writes=%d",
			credentials.reads, credentials.writes)
	}
}
...
func TestRunnerMarketingDiscoveryRejectsInvalidInputBeforeState(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			stdout := new(bytes.Buffer)
			runner := newMarketingDiscoveryRunner(
				t, root, store.path, transport, store, credentials, stdout,
			)
			...
			if reads != 0 || writes != 0 || len(credentials.reads) != 0 || credentials.writes != 0 ||
				len(transport.snapshot()) != 0 {
				t.Fatalf("invalid input crossed boundary: config=%d/%d credentials=%v/%d HTTP=%d",
					reads, writes, credentials.reads, credentials.writes, len(transport.snapshot()))
			}
		})
	}
}

func TestDefaultRouteUsesMarketingDiscoveryGo(t *testing.T) {
