...
func (probe *runnerEnvironmentProbe) F2(context.Context) onboarding.Check {
	return probe.check("f2", true)
}
...
func TestTopLevelDomainAndCommandHelpStayInGo(t *testing.T) {
	for _, args := range [][]string{{"--help"}, {"accounts", "--help"}, {"accounts", "list", "--help"}} {
		...
		if !strings.Contains(stdout.String(), "Usage: ocean-watch") {
			t.Fatalf("help missing for %v: %s", args, stdout.String())
		}
	}
}

func TestAccountListUsesGoRuntime(t *testing.T) {
...
func TestSetupDoctorUsesGoOnceWithoutCredentialsOrConfigWrites(t *testing.T) {
	...
	wantCalls := []string{"python", "f2", "platform", "codex_cli", "credential_backend", "oauth_callback"}
...
func TestSetupValidateUsesOnlyReadOnlyLocalState(t *testing.T) {
  ...
  "plan_template_schema_version": 6
	...
	wantReads := []string{
		"oceanengine-app-marketing", "oceanengine-auth-marketing-auth-1-r1",
	}
...
func TestSetupInitCreatesPreservesAndForceReplacesOnlyTargetConfig(t *testing.T) {
	...
	for _, field := range []string{
		...
		"oauth_setup", "minimum_fields_for_query_data",
	...
	if credentials.writes != 0 {
		t.Fatalf("setup init wrote credentials %d times", credentials.writes)
	}
	...
	if credentials.writes != 0 {
		t.Fatalf("unexpected init credential writes: %d", credentials.writes)
	}
	if len(probe.calls) != 18 {
		t.Fatalf("environment probe count = %d, want 18", len(probe.calls))
	}
...
func TestSetupExitCodesRemainCompatible(t *testing.T) {
	...
	doctorRunner := Runner{
		Routes: application.DefaultRouteManifest(), Stdout: doctorOutput,
		Cwd: root, UserHome: root, Getenv: func(string) string { return "" },
		Credentials:      &runnerCredentialStore{entries: map[string]map[string]any{}},
		Authorizations:   &runnerAuthorizationReader{states: map[string]domain.AuthorizationState{}},
...
func TestResolveSkillRootUsesPluginRoot(t *testing.T) {
	pluginRoot := filepath.Join(t.TempDir(), "plugin")
	if got := resolveSkillRoot(pluginRoot); got != filepath.Join(pluginRoot, "skills", "ads-plan-monitor") {
		t.Fatalf("plugin skill root = %q", got)
	}
	if got := resolveSkillRoot(""); got != "" {
		t.Fatalf("empty plugin root resolved to %q", got)
	}
}
...
func TestAccountRemoveMatchesStableResponseShape(t *testing.T) {
	path := filepath.Join(t.TempDir(), "config.json")
...
func TestRunListUsesManagedStateRoot(t *testing.T) {
	codexHome := t.TempDir()
