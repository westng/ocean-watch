...
func TestRunnerMarketingReportMapsCompleteArguments(t *testing.T) {
	root := t.TempDir()
	...
	configReads := 0
	runner := Runner{
		Routes: marketingReportGoManifest(t),
	...
	if configReads != len(tests) {
		t.Fatalf("report mapping config reads = %d, want %d", configReads, len(tests))
	}
...
func TestRunnerMarketingReportRuntimeUsesGeneratedSDKAndIsolatedCredentials(t *testing.T) {
	root := t.TempDir()
	configPath := writeMarketingReportCLIConfig(t, root)
	...
	newRunner := func(stdout io.Writer) Runner {
		return Runner{
			Routes: marketingReportGoManifest(t), Stdout: stdout,
	...
	customCSV := filepath.Join(root, "runtime-custom.csv")
	tests := []struct {
		name   string
		args   []string
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			outputPath := filepath.Join(root, "runtime-"+test.name+".json")
			args := append(append([]string(nil), test.args...), "--out", outputPath)
			stdout := new(bytes.Buffer)
			if code := newRunner(stdout).Execute(context.Background(), args); code != 0 {
				t.Fatalf("%s Runtime exit = %d: %s", test.name, code, stdout.String())
			}
	...
	if configReads != len(tests) {
		t.Fatalf("Marketing report config reads = %d, want %d", configReads, len(tests))
	}
...
func TestRunnerMarketingReportRejectsInvalidArgumentsBeforeState(t *testing.T) {
	...
	var configMu sync.Mutex
	runner := Runner{
		Routes: marketingReportGoManifest(t), Credentials: credentials,
		Cwd: root, UserHome: root, Getenv: func(string) string { return "" },
		MarketingReports: MarketingReportRuntime{
			Service: service,
			ConfigFactory: func(path string) MarketingReportConfigReader {
	...
	if configReads != 0 || len(credentials.reads) != 0 || credentials.writes != 0 || serviceCalls != 0 {
		t.Fatalf("invalid reports crossed boundary: config=%d credentials=%v/%d service=%d",
			configReads, credentials.reads, credentials.writes, serviceCalls)
	}
}

func TestMarketingReportCSVEmptyResultMatchesStableShape(t *testing.T) {
...
func TestDefaultRouteUsesMarketingReportsGo(t *testing.T) {
	for _, command := range []string{
		"reports schema", "reports custom", "reports plans", "reports materials",
	} {
