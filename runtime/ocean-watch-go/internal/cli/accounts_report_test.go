...
func TestRunnerAccountReportRuntimeAssemblesTokenAndSDKAdapters(t *testing.T) {
	root := t.TempDir()
  ...
  "managed_accounts": {
	...
	if code != 0 {
		t.Fatalf("account report runtime exit = %d: %s", code, stdout.String())
	}
	var result domain.AccountReportResult
	if err := json.Unmarshal(stdout.Bytes(), &result); err != nil {
		t.Fatal(err)
	...
	if !result.OK || len(result.Accounts) != 2 || len(result.Presentation.Columns) != 13 {
		t.Fatalf("account report runtime returned incomplete result: %#v", result)
	}
	transport.mu.Lock()
	...
	if !reflect.DeepEqual(calls, wantCalls) {
		t.Fatalf("account report runtime calls = %#v, want %#v", calls, wantCalls)
	}
	for _, call := range calls {
		if strings.Contains(call.Path, "/qianchuan/uni_promotion/list/") {
			t.Fatalf("account report runtime called Qianchuan plan list: %#v", call)
		}
	}
...
func TestDefaultRouteUsesAccountReportGo(t *testing.T) {
	runtime, ok := application.DefaultRouteManifest().RouteFor("accounts report")
	if !ok || runtime != application.RuntimeGo {
		t.Fatalf("default accounts report route = %q, want Go", runtime)
	}
}
